...
DATA_DIR = os.path.join(
    os.path.dirname(__file__), "data", "DI_CONNECT", "DI-Connect-Wellness"
)
...
def load(data_dir: str, data_name: str) -> pd.DataFrame:
    ...
    if not files_path:
        raise FileNotFoundError(
            f"No files named: '{data_name}.json' found in {data_dir}"
        )
    ...
    # Flat the data
    flat_data = [
        item for sublist in data if isinstance(sublist, list) for item in sublist
    ]
...
def clean_data(
    df: pd.DataFrame, columns_mapper: dict, drop_cols: list, sec_hours: bool
) -> pd.DataFrame:
    """
...
def engineer_features_sleep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["total_sleep_hours"] = (
        df["deep_sleep_hours"] + df["light_sleep_hours"] + df["rem_sleep_hours"]
    )
